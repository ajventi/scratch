...
function New-RackingWorksheet {
    ...
    $FilledContainers | ForEach-Object {
        $CommandText = "INSERT INTO bulk_wine (fill_date, blend_id, Container_ID, Volume) SELECT $Date, $blendIds,"
        if ($_.GetType() -is [string]) {
            $CommandText += "$_, NULL"
        } else {
            $CommandText += "'$($_.id)', $($_.volume)"
        }
        $CommandText += " RETURNING container_id, id;"
        $foo = Import-DBQuery -Transaction $trans $CommandText
    }
    # I don't get WTF I am getting the query returned and not the transaction
    return $trans
}
